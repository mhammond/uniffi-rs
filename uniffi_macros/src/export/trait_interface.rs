...
pub(super) fn gen_trait_scaffolding(
    ...
    let trait_impl = with_callback_interface.then(|| {
    ...
    let meta_static_var = (!udl_mode).then(|| {
        let imp = if with_callback_interface {
    ...
    let ffi_converter_tokens =
        ffi_converter(mod_path, &self_ident, udl_mode, with_callback_interface);
...
pub(crate) fn ffi_converter(
    ...
    let metadata_code = if with_callback_interface {
