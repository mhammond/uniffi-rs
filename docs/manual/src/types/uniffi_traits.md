...
This is supported primarily for `Interface`s - it's also supported for `Record`s and `Enum`s, but it has fewer use-cases there - see [data-classes](#data-classes) below.
...
For example, all bindings will automatically generate a simple equality check and hash function for records which simply compare/hash each element in the record.
...
but it's there if you need it for obscure cases where you really do need custom functionality and the performance hit is OK!
