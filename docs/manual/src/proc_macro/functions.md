...
`default` inputs a comma-separated list of either `[name]` items (where the type's default value should be used)
...
- All `Option` types default to `None`, Vecs and Maps default to empty items
